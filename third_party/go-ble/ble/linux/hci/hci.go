...
import (
	...
	"github.com/Rightech/ric-edge/third_party/go-ble/ble/linux/hci/cmd"
	"github.com/Rightech/ric-edge/third_party/go-ble/ble/linux/hci/evt"
	"github.com/Rightech/ric-edge/third_party/go-ble/ble/linux/hci/socket"
...
func (h *HCI) handleEvt(b []byte) error {
	...
	if plen != len(b[2:]) {
		err = fmt.Errorf("invalid event packet: % X", b)
	}
	if f := h.evth[code]; f != nil {
		err = f(b[2:])
		return nil
